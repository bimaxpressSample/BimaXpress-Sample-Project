...
const OverView = () => {
    ...
    const fetchCurrentPlan = async () => {
        ...
        try {
            ...
            console.log("subreference" , data?.data?.data[0]?.subReferenceId);

            if(data?.data?.data[0]?.subReferenceId != ''){
                const AUTHURL = `/getsubscriptiondetailsfromcashfree?subReferenceId=${data?.data?.data[0]?.subReferenceId}` ;
                console.log("AUTHURL" , AUTHURL);
                //@ts-ignore
                const  getsubscription  = await axiosConfig.get(AUTHURL)
                // setauthLink(subscription);
                console.log("get details",getsubscription?.data?.data?.subscription);
                setauthLinkData(getsubscription?.data?.data?.subscription);
            }
        } catch (error) {
            dispatch(setLoading(false));
            //@ts-ignore
    ...
    console.log("auth link ", authLinkData);
    ...
    return (
        <div className="w-auto">
            ...
            {
                // @ts-ignore
                currentPlanDetails?.subReferenceId != "" && authLinkData?.status === "INITIALIZED" ? (
                    <div className={`${styles.overviewContainer} flex justify-center items-center`}>
                        {/* @ts-ignore */}
                        <p className="text-lg text-fontColor font-semibold capitalize" >Your E-Mandate Is InActive Kindly Connect With Sales Team Or <a target='_blank' href={authLinkData?.authLink} className="font-semibold underline hover:text-yellow-800 dark:hover:text-yellow-900">Click Here</a></p>
                    </div>
                ):(
                    <div>
