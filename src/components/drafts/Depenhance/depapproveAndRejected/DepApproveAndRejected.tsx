...
const ApproveModal = ({
  ...
  console.log("case data in arropve rejected" , newCaseData );
  ...
  const uploadFile = async () => {
    ...
    const URLFORMCREATIONAUDITTRIAL = `/${
      //@ts-ignore
      action === "Enhance" ? enhanceApi[status] : fciApi[status]}?email=${newCaseData?.hospital_email}&casenumber=${newCaseData?.caseNumber}`;
  ...
  return (
    <div className={styles.approveModalContainer}>
      <div className="px-10 py-8 relative" style={{ height: '70vh', overflow: 'auto' }} >
        {/* <IoClose
