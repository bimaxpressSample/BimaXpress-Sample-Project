              ...
              <tr {...headerGroup.getHeaderGroupProps()}>
                {headerGroup.headers.map((column) => (
                  <th
                    ...
                    className={`bg-secondary py-3 px-4 text-xs font-semibold text-fontColor text-left ${styles.tableHeader}`}
                  >
              ...
              <tr {...row.getRowProps()}>
                {row.cells.map((cell) => {
                  return (
                    <td
                      {...cell.getCellProps()}
                      className='px-4 pt-5 pb-12 border-b border-fontColor-darkGray text-xs text-fontColor font-thin last:border-b-0'
                    >
                      {cell.render('Cell')}
                    </td>
                  );
