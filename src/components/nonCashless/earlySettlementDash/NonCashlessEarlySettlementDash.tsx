...
import styles from './NonCashlessEarlySettlementDash.module.css';

interface ColumnDetails {
  [key: string]: any;
}

const NonCashlessEarlySettlementDash = () => {
  ...
  const fetchESDashboardData = async () => {
    ...
    try {
      await axiosConfig.post(`/NCP_Es_dashboard?email=${user}`);
    } catch (error) {
    ...
    try {
      const { data } = await axiosConfig.get(`/NCP_Es_dashboard?email=${user}`);
      dispatch(setLoading(false));
    ...
    if (Object.entries(esData)?.length) {
      const res = Object.entries(esData)?.map(
        (
          ...
          es_status: es_status,
          offerAvailDate: new Date(Offer_date).toLocaleDateString(),
          settleAmt: settled,
          dateOfSettle: new Date(repayment_date).toLocaleDateString(),
...
export default NonCashlessEarlySettlementDash;
